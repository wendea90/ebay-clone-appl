...
export default function TopMenu() {
    ...
    //for add and remove cart from cart.js
    const cart = useCart();

    //state for log in - then  onclick happen on button
    const [isMenu, setIsMenu] = useState(false)

    //make a function called logged in
    const isLoggedIn = () => {
        //if the user and user id is empty return
        if (user && user?.id) {
            return (
                <button
        ...
        //if this does not trigger retur
        return (
            ...
            <div id="TopMenu" className="border-b">
                <div className="flex items-center justify-between w-full mx-auto max-w-[1200px]">
                    ...
                    >
                        ...
                        <li className="px-3 hover:underline cursor-pointer text-sm">
                            Daily Deals
                        </li>
                        <li className="px-3 hover:underline cursor-pointer text-sm">
                            Help & Contact
                    ...
                    >
                        <li
                            ...
                            className="flex items-center gap-2 px-3 hover:underline cursor-pointer text-sm"
                        >
                        ...
                        <ClientOnly>
                            <li className="px-3 hover:underline cursor-pointer">
                                <div
                                    onClick={() => router.push('/cart')} className="relative">
                                    <AiOutlineShoppingCart size={22} />

                                    {/* if the cart count is greater than 0 return count cart */}
                                    {cart.cartCount() > 0 ?

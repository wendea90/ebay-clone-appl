...
export default function Checkout() {
    ...
    //2.variables to use as reference
    ...
    //4.effects happen
    //A toast notification is a small, often temporary pop-up that provides feedback to the user. This feedback can be in the form of success messages, warnings, errors, or informational messages.
    useEffect(() => {
        //if cart&cart total is less than or equal to zero...
        if (cart?.cartTotal() <= 0) {
    ...
    const pay = async (event) => {
        ...
        if (result.error) {
            ...
            try {
                //to save the order items after we have made successfull card payment
                                ...
                                <div>
                                    {!isLoadingAddress ?
                                        <Link href="/address" className="text-blue-500 text-sm underline">
                                            {/* if there is a name for the addtess details we want to update address if there is not & if isloading address false */}
                                            {addressDetails.name ? 'Update Address' : 'Add Address'}
                                        </Link>
                                        : null}

                                    {/*if is loading address is false & address details name exists else null*/}
